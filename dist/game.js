...
function setView(){ $('canvas').hidden=flat;$('flat').hidden=!flat;$('view').textContent=flat?'Use 3D board':'Use 2D board';$('instructions').textContent=flat?'Select a piece, then a highlighted square. Arrow keys navigate the board.':'Select a piece, then a highlighted square. Scroll to zoom. Flip board to change sides.';render() }
let has3D=false;
...
try{
 ...
 for(const flip of [false,true])for(const color of ['w','b'])for(const [column,type] of ['p','n','b','r','q','k'].entries()){
  ...
  const plate=label(rankMarks[type],.42,.13,'#fff0bd','#101820');plate.quaternion.copy(camera.quaternion);plate.position.y=.12;g.add(plate);
